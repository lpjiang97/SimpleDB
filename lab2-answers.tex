...
\begin{enumerate}[label=\textbf{\arabic*.}, listparindent=0.0em, itemsep=1em]
	...
	\begin{itemize}
		...
		\item \texttt{BufferPool}: We added \texttt{insert}, \texttt{delete} in it, which insert/delete tuples to/from the
            ...
            page to disk and add the new page into the \texttt{BufferPool}
	\end{itemize}
    \par Examples:
    ...
    \item Design decisions: I chose to use nested loop join. For eviction policy, I chose to evict the first page every
        time the \texttt{BufferPool} is full.
